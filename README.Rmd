...
# Call scrappy
out_newa <- scrappy::newa_nrcc(client = rD$client, 
                               year = 2020, 
                               month = 12, # December
                               station = "gbe", # Geneva (Bejo) station
                               save_file = FALSE) # Don't save output

out_gmaps <- scrappy::google_maps(client = rD$client,
                                  name = "Sefton Park",
                                  max_reviews = 20)
# Stop server
conn <- rD$server$stop()
...
`%>%` <- scrappy::`%>%`
out_gmaps %>%
  dplyr::mutate(author = paste0("Author ", seq_along(author)),
                author_url = "") %>%
knitr::kable()
```
